...
def CheckLotterNum(lotter_num, bool_reLotter):
    ...
    if lotter_num[0] > 0 and lotter_num[0] <= 45 and lotter_num[0] != lotter_num[1] and lotter_num[0] != lotter_num[2] and lotter_num[0] != lotter_num[3] and lotter_num[0] != lotter_num[4] and lotter_num[0] != lotter_num[5] :
        ...
        print("\n로또 당첨 번호 적합하지 않습니다. 로또 당첨 번호를 입력해주세요. 동일하지 않은 1 ~ 45 사이의 숫자입니다")
...
###########로또 당첨 번호 입력받기
print("이번 회차 로또 당첨 번호를 입력해주세요 1 ~ 45 사이의 숫자입니다")
while(bool_reLotter) :
    try:
        lotter_num[0] = int(input("1번째 로또 당첨 번호는(1~45) = "))
        lotter_num[1] = int(input("2번째 로또 당첨 번호는(1~45) = "))
        lotter_num[2] = int(input("3번째 로또 당첨 번호는(1~45) = "))
        lotter_num[3] = int(input("4번째 로또 당첨 번호는(1~45) = "))
        lotter_num[4] = int(input("5번째 로또 당첨 번호는(1~45) = "))
        lotter_num[5] = int(input("6번째 로또 당첨 번호는(1~45) = "))
    except ValueError:
        print("\n로또 당첨 번호를 정수로 입력하세요.")
    else:
        #로또 당첨 번호가 규칙에 맞는지 체크, 중복안되고, 1~45사이의 정수
        bool_reLotter = CheckLotterNum(lotter_num, bool_reLotter)


print("이번회차 로또 당첨 번호는 {0} {1} {2} {3} {4} {5}".format(lotter_num[0],lotter_num[1],lotter_num[2],lotter_num[3],lotter_num[4],lotter_num[5]))
...
while (bool_end):
    ...
    #랜덤 숫자 위치 초기화
    intRdnum = 0   #이번 고객의 6개 랜덤 로또번호 중 순서 
    intEqnum = 0   #이번로또중 동일한 숫자가 있는 체크에 사용, 전에 뽑은 숫자와 동일하면 다시 뽑음

    #이번고객의 랜덤 로또 숫자 발생 루틴
    while (bool_rand) :
        int_rand[intRdnum] = randint(1,45) #랜덤로또숫자 발생
 ...
 #   print(str(cnt_rount)+" 번째 롯또숫자는 = "+str(int_rand[0])+" "+str(int_rand[1])+" "+str(int_rand[2])+" "+str(int_rand[3])+" "+str(int_rand[4])+" "+str(int_rand[5])+" ")
    ...
    if cntLotter == 5 : #5개 맞으면 출력
        cntFive += 1
        print("{0:<10,}번째 고객님 5개 숫자 맞음, 고객님의 로또번호는 = ".format(cnt_rount+1),end=" ")
        print("{0:<3} {1:<3} {2:<3} {3:<3} {4:<3} {5:<3}".format(int_rand[0],int_rand[1],int_rand[2],int_rand[3],int_rand[4],int_rand[5]))
...
#########  마무리 정리 프린트
print("\n이번회차 1등 당첨은 {0:<10,}번째 고객님께 돌아갔습니다.".format(cnt_rount))
print("1등 로또숫자는 = "+str(int_rand[0])+" "+str(int_rand[1])+" "+str(int_rand[2])+" "+str(int_rand[3])+" "+str(int_rand[4])+" "+str(int_rand[5])+" ")
print("로또번호 3개 맞은 고객은 {0:<10,}명이며, {1:.4f}%의 당첨확률 입니다.".format(cntThree, cntThree/cnt_rount*100))
print("로또번호 4개 맞은 고객은 {0:<10,}명이며, {1:.4f}%의 당첨확률 입니다.".format(cntFour, cntFour/cnt_rount*100))
print("로또번호 5개 맞은 고객은 {0:<10,}명이며, {1:.4f}%의 당첨확률 입니다.".format(cntFive, cntFive/cnt_rount*100))

input("로또에 당첨될 확률을 알아봤습니다. 아무키나 누르세요")
print("Good Bye 로또")